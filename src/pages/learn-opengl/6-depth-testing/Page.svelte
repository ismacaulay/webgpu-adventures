<script lang="ts">
  ...
  onMount(() => {
    ...
    (async () => {
        ...
        const modelBuffer = bufferManager.createUniformBuffer(
          {
            ...
            render_mode: UniformType.Scalar,
          },
          {
            model: mat4.create(),
      ...
      pane
        .addInput(params, 'renderMode', {
          options: {
            normal: RenderMode.Normal,
            depth: RenderMode.Depth,
            'linearized depth': RenderMode.LinearizedDepth,
          },
        })
        .on('change', () => {
          allShaders.forEach((id) => {
            const shader = shaderManager.get(id);
            shader.update({ render_mode: params.renderMode });
          });
        });
    })();
